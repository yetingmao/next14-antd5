...
export default function deviceDetail() {
    ...
    if (detail.taskType == 2) {
        resultEle = dataList?.map((item, index) => (
            <div key={item.fileId || index} style={{ display: "flex" }}>
                {item.washStatus !== 3 ? <Card
                    size="small"
                    hoverable={true}
                    title={<Checkbox disabled={item.washStatus !== 5} value={item.fileId}></Checkbox>}
    ...
    console.log("------",checkedList,fileIds)
    return (
    ...
    async function getList(id) {
        ...
        if (res?.code == 200) {
            let { rows, total } = res;
            setDataList(rows)
            const ids = rows.filter(item => item.washStatus == 5).map(item => item.fileId)
            setFileIds(ids)
            setTotal(total)
        } else {
            return
