...
export default function () {
        ...
        },
        {
            ...
            render: (text, record) => {
                ...
                const push = <Button
                    type="link"
                    onClick={() => {
                        router.push(`/cleanCheck/check?id=${record["id"]}`)
                    }}
                >审核</Button>;
                let ele;
                if (text == 11) {
                    ...
                    <div className="top" ref={(node) => {
                        ...
                        >
                            <Form.Item
                                ...
                                label={`任务编号`}
                            >
                                <Input placeholder="任务编号" style={{ width: 200 }} />
                            </Form.Item>
                            <Form.Item
                                name={`taskName`}
                                label={`任务名称`}
    ...
    async function getList() {
        ...
        const temp = _.pickBy({ ...values, assetStartTime, assetEndTime, taskType: "1", pageSize: pagination.pageSize, pageNum: pagination.current }, _.identity);
