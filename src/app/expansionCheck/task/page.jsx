...
export default function () {
            ...
            render: (text, record) => {
                ...
                const details =
                    <Button
                        type="link"
                        onClick={() => {
                            router.push(`/expansionCheck/details?id=${record["id"]}`)
                        }}
                    >详情</Button>
                const push = <Button
                    type="link"
                    onClick={() => {
                        router.push(`/expansionCheck/check?id=${record["id"]}`)
                    }}
                >审核</Button>;
                let ele;
                if (text == 8) {
    ...
    async function getList() {
        ...
        const temp = _.pickBy({ ...values, examineStartTime, examineEndTime, taskType: "3", pageSize: pagination.pageSize, pageNum: pagination.current }, _.identity);
