...
import {
    getModelList, getCategoryList, getTagTreeAll, getTaskList, delTask, taskAction
} from "../api.js";
...
export default function () {
        ...
        },
        {
            ...
            render: (text, record) => {
                const self = <Button
                    type="link"
                    onClick={() => {
                        action({ taskId: record["id"], type: 1 })
                    }}
                >自身清洗</Button>;
                const all = <Button
                    type="link"
                    onClick={() => {
                        // action({ taskId: record["id"], type: 2 })
                        router.push(`/sampleClean/selfClean?id=${record["id"]}&taskType=${record["taskType"]}`)
                    }}
                >全库清洗</Button>;
                const details =
                    <Button
                        type="link"
                        onClick={() => {
                            router.push(`/sampleClean/allClean?id=${record["id"]}&taskType=${record["taskType"]}`)
                        }}
                    >详情</Button>
                const details1 =
                    <Button
                        type="link"
                        onClick={() => {
                            router.push(`/sampleClean/taskDetails?id=${record["id"]}&taskType=${record["taskType"]}`)
                        }}
                    >详情</Button>
                const edit = <Button
                    type="link"
                ...
                const pause = <Button
                    type="link"
                    onClick={() => {
                        action({ taskId: record["id"], type: 3 })
                    }}
                >暂停</Button>;
                const keep = <Button
                    type="link"
                    onClick={() => {
                        action({ taskId: record["id"], type: 5 })
                    }}
                >继续</Button>;
                const end = <Button
                    type="link"
                    onClick={() => {
                        action({ taskId: record["id"], type: 4 })
                    }}
                >结束</Button>;
                ...
                <Col span={24}>
                    <div className="top" ref={(node) => {
                        ...
                        >
                            ...
                            {/* <Form.Item label="清洗方式" name={`desensitizationWay`} >
                                <Select style={{ width: 200 }} options={[
                                    {
                                        value: '',
                                        label: '全部',
                            ...
                            {/* <Form.Item label="清洗类别" name="modelCategoryId">
                                <Select mode="multiple" allowClear options={[{
                                    value: "",
                                    label: "全部",
                                }, ...categoryList]} style={{ width: 200 }}> </Select>
                            </Form.Item> */}
                            <Form.Item label="清洗路径" name={`tagIds`} >
                                <TreeSelect
                                    // multiple
                                    treeDataSimpleMode
                                    style={{
                            ...
                            <Form.Item label="是否提交审核" name={`submit`} >
                                <Select style={{ width: 200 }} options={[
                                    {
                                        value: '',
                                        label: '全部',
    ...
    async function getList() {
        ...
        const temp = _.pickBy({ ...values, startTime, endTime, taskType: "2", pageSize: pagination.pageSize, pageNum: pagination.current }, _.identity);
    ...
    async function getParameter() {
        ...
        res = await getModelList({ type: 2, pageNum: 1, pageSize: 100 });
        if (res?.code == 200) {
        ...
        function traverse(obj, parentPath = []) {
            if (typeof obj === 'object' && obj !== null) {
                if ('name' in obj) {
                    ...
                    if ('children' in obj && Array.isArray(obj.children) && obj.children.length) {
