...
export default function deviceDetail() {
    ...
    if (detail.executionMethod == 1) {
        resultEle = dataList?.map((item, index) => (
            <div key={index} style={{ display: "flex", background: "rgb(240, 242, 245)", marginTop: "5px" }}>
                ...
                {item.childFiles ? item.childFiles.map((item, index) => (
                    <Card
                        key={index}
                        size="small"
                        hoverable={true}
                        title={<Checkbox value={item.fileId} disabled={item.status != 0}>扩充后图</Checkbox>}
                        extra={<Button type="link" icon={<DownloadOutlined onClick={() => {
                            const a = document.createElement("a");
                            a.href = item.fileUrl;
                        ...
                        >
                            ...
                            >
                                <Descriptions bordered>
                                    <Descriptions.Item label="扩充任务编号">{detail.id || "暂无"}</Descriptions.Item>
                                    <Descriptions.Item label="扩充任务名称">{detail.taskName || "暂无"}</Descriptions.Item>
                                    <Descriptions.Item label="任务状态">
                                        <Tag icon={<CheckCircleOutlined />} color="success">{detail.statusResult}</Tag>
                                    </Descriptions.Item>

                                    {/* <Descriptions.Item label="扩充类别">
                                        {detail.modelCategoryResults ? detail.modelCategoryResults.join(",") : "暂无"}
                                    </Descriptions.Item> */}
                                    <Descriptions.Item label="扩充模型">{detail.modelName ? detail.modelName.join(",") : "暂无"}</Descriptions.Item>
    ...
    async function getDetail(id) {
        ...
        if (code == 200) {
            setDetail(data)
            const { modelCategoryIds, modelCategoryResults, desensitizationWay, status, executionMethod } = data;
            if (status == 7) {
                router.back();
                return;
            }
            ...
            getList(id, executionMethod)
            setTaskQuery({ ...taskQuery, taskId: id, executionMethod })
        } else {
            message.error(msg)
        }
    }
    async function getList(id, type) {
        //  setLoading(true);
        let executionMethod = type || taskQuery.executionMethod;
        ...
        if (res?.code == 200) {
            ...
            if (executionMethod == 1) {
                rows.forEach(item => {
                    if (item.childFiles) {
                        item.childFiles.forEach(child => {
                            ids.push(child.fileId)
                        })
                    }
                });
            } else {
                ids = rows.filter(item => item.status == 0).map(item => item.fileId)
            }
            setFileIds(ids)
            setTotal(total)
        } else {
