...
export default function () {
    ...
    useEffect(() => {
        if (_fNode && _bNode1 && _bNode2) {
            setTHeight(Util.initTableHeight(_fNode, [_bNode1, _bNode2], 180)); //操作按钮，分页，padding值40
        }
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        >
                            <Input addonBefore="从" addonAfter="张" />
                        </Form.Item>
                        <Form.Item
                            name="endIndex"
                            rules={[
                        ...
                        >
                            <Input addonBefore="至" addonAfter="张" />
                        </Form.Item>
                    </Form.Item> : ""}
                </Form>
                <List
    ...
    async function getList() {
        ...
        const temp = _.pickBy({ ...values, dataType: 2, importStartTime, importEndTime, pageSize: pagination.pageSize, pageNum: pagination.current }, _.identity);
    ...
    async function exportData() {
        exportform.validateFields().then(async (values, e, r) => {
            ...
            url += `?${Qs.stringify({ dataId: data.id, exportPurpose: values.exportPurpose })}`;
            Util.download(url);
            setModelShow1(false)
        });
    };
