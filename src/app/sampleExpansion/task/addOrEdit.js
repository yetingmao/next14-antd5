...
export default function ({ data, modelShow, setModelShow,
  ...
  useEffect(() => {
    form.setFieldsValue({
      executionMethod: data.executionMethod ?? 1,
      id: data.id ?? "",
      ...
      time: data.startTime ? [dayjs(data.startTime, 'YYYY-MM-DD HH:mm:ss'), dayjs(data.endTime, 'YYYY-MM-DD HH:mm:ss')] : undefined,
    })
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Input  cautocomplete="off" placeholder="扩充模型名称" />
        </Form.Item>
        <Form.Item label="扩充路径" name={`tagIds`} rules={[
          {
            required: true,
            message: '标签必填!',
          },
        ...
        <Form.Item label="扩充模型" name={`desensitizationManageId`} rules={[
          {
            required: true,
            message: '扩充模型必填!',
          },
        ...
        {/* {show ? <Form.Item label="扩充类别" name="modelCategoryIds" rules={[
          {
            required: true,
            message: '扩充类别必填!',
          },
        ...
        <Form.Item label="扩充方式" name="executionMethod" >
          <Radio.Group disabled={data.id}>
            <Radio value={1}> 依次扩充 </Radio>
            <Radio value={2}> 叠加扩充 </Radio>
          </Radio.Group>
        </Form.Item>
  ...
  async function addOrUp() {
    form.validateFields()
      .then(async (values, e, r) => {
        ...
        if (values.id) {//修改
          const res = await updateTask({ ...values, startTime, endTime, taskType: 3 })
          if (res?.code == 200) {
          ...
          const res = await addTask({ ...values, startTime, endTime, taskType: 3 })
          if (res?.code == 200) {
            message.success(res.msg)
            setConfirmLoading(false)
            reset()
